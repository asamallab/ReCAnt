## Title: Chronic SSD computations
## Date: 23-12-2024
...
sink(file="chronic_code_full_log.txt", split=TRUE)
...
save_path = file.path(home, "ssd_output", "chronic")
...
################################################################################
# chronic SSDS
# set of chemicals for chronic SSD construction
ssd_chemicals_chronic = subset(x=ssd_chemicals, subset = Type=='chronic')

# extract their CAS
chronic_chemicals = ssd_chemicals_chronic$Chemical
print(sprintf("Number of CAS ids: %s", length(chronic_chemicals)))
print("Top five chemical CAS")
print(chronic_chemicals[1:5])

# extract their names
chronic_chemicals_names = ssd_chemicals_chronic$Chemical_name
print(sprintf("Number of chemicals names: %s", length(chronic_chemicals_names)))
print("Top five chemicals' names")
print(chronic_chemicals_names[1:5])



# number of chemicals
loop_iter = length(chronic_chemicals)
...
output_df <- foreach(i = 1:loop_iter, 
   ...
   temp_log <- c(sprintf("Constructing SSD for %s", chronic_chemicals[i]))
   
   # for timing the speed
   start_time <- Sys.time()
   
   # read the dataframe
   df_path <- file.path(home, "ssd_input", "chronic", sprintf("%s.csv", chronic_chemicals[i]))
   df <- read.csv(df_path)
   colnames(df) <- make.names(colnames(df))
   temp_log <- c(temp_log, sprintf("Input file read"))
   
   # create folder for the chemical
   if (!dir.exists(file.path(save_path, sprintf("%s", chronic_chemicals[i])))) {
     dir.create(file.path(save_path, sprintf("%s", chronic_chemicals[i])))
   }
   ...
   lbl <- label_size_fn(chronic_chemicals[i], ssd_chemicals_chronic)
   ...
   chem_all_models <- plot_allmodels(df, cas=chronic_chemicals[i], cname=chronic_chemicals_names[i], lbls = lbl_size)
   
   if (is.null(chem_all_models)) {
     temp_log <- c(temp_log, sprintf("Skipping %s: plot_allmodels failed", chronic_chemicals[i]))
     log_file <- file.path(save_path, sprintf("%s", chronic_chemicals[i]), sprintf("log_file_chronic_SSDs_%s.txt", chronic_chemicals[i]))
     writeLines(temp_log, log_file)
     return(data.frame(chemical=chronic_chemicals[i], execution_time=NA))
   }
   ...
   plot_averagemodel(df, df_fits, cas=chronic_chemicals[i], cname=chronic_chemicals_names[i],lbls = lbl_size)
   temp_log <- c(temp_log, sprintf("Averaged model plots generated and saved"))
   
   # extract and save HC05 data
   get_hc05_data(df_fits, cas=chronic_chemicals[i])
   temp_log <- c(temp_log, sprintf("HC05 data generated and saved"))
   
   # plot best fit model
   plot_bestfit(bfit, df, cas=chronic_chemicals[i], cname=chronic_chemicals_names[i], lbls = lbl_size)
   temp_log <- c(temp_log, sprintf("Best-fit model plots generated and saved"))
   
   diff_time <- difftime(Sys.time(), start_time, units = "mins")
   temp_log <- c(temp_log, sprintf(sprintf("SSD construction complete for %s with an execution time (in mins) of %s",chronic_chemicals[i], diff_time)))
   temp_log <- c(temp_log, sprintf("==============================="))
   
   log_file <- file.path(save_path, sprintf("%s", chronic_chemicals[i]), sprintf("log_file_chronic_SSDs_%s.txt", chronic_chemicals[i]))
   writeLines(temp_log, log_file)
   
   return(data.frame(chemical=chronic_chemicals[i], execution_time=diff_time))
}
